...
public class ConfigManager {
            ...
            for (String colorName : colorsSection.getKeys(false)) {
                try {
                    ...
                    if (colorSection == null) {
                        plugin.getLogger().warning("Invalid color section for: " + colorName);
                        skipped++;
                        continue;
                    }
                    ...
                    if (validateHexCodes && !isValidHexCode(hexCode)) {
                        plugin.getLogger()
                                .warning(
                                        "Invalid hex code for color '" + colorName + "': " + hexCode + ". Skipping...");
                        skipped++;
                        continue;
                    }
                    ...
                    plugin.getLogger().severe("Error loading color '" + colorName + "': " + e.getMessage());
                    skipped++;
                }
            }

            plugin.getLogger().info("Loaded " + loaded + " colors from configuration" +
                    (skipped > 0 ? " (" + skipped + " skipped due to errors)" : ""));

            if (availableColors.isEmpty()) {
                plugin.getLogger().warning("No valid colors were loaded! Creating default colors...");
                createDefaultColors();
            }
        } catch (Exception e) {
            plugin.getLogger().severe("Critical error loading color configuration: " + e.getMessage());
            e.printStackTrace();
            createDefaultColors();
        }
    }
    ...
    public java.util.List<String> validateConfiguration() {
        ...
        for (Map.Entry<String, java.util.List<String>> entry : hexToNames.entrySet()) {
            if (entry.getValue().size() > 1) {
                issues.add("Duplicate color codes found for " + entry.getKey() + ": "
    ...
    public static class ColorStats {
        ...
        public int getPublicColors() {
            return publicColors;
        }

        public int getProtectedColors() {
